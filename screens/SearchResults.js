      ...
      return (
        <View
          ...
          contentContainerStyle={styles.contentContainer}>
            <View>
              <Text style={styles.resultsTitleText}>Search Results: {query.slice(0,1).toUpperCase() + query.slice(1)}</Text>
            </View>
            <ScrollView>
              {Object.keys(response).map( (title, index) => {
                let id = response[title]['id']
                return(
                  <TouchableOpacity style={styles.recipeCard} onPress={_ => this.submitShowIngredients(id)} key={title + index}>
